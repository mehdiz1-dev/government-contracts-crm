  ...
  useEffect(() => {
    ...
    const fetchContract = async () => {
      ...
      try {
        ...
        setFormData({
          ...data,
          bc_date: data.bc_date ? new Date(data.bc_date).toISOString().split('T')[0] : '',
          delivery_deadline_date: data.delivery_deadline_date ? new Date(data.delivery_deadline_date).toISOString().split('T')[0] : '',
          contract_value: data.contract_value !== null ? String(data.contract_value) : '', // Convert Decimal to string
        });
